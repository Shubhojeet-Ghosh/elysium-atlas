...
function Slider({
  ...
  // Track dragging thumb index for tooltip
  const [dragging, setDragging] = React.useState(-1);
  // Track which thumb is hovered (for multi-thumb support)
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <Tooltip key={index} open={hovered === index || dragging === index}>
          <TooltipTrigger asChild>
            <SliderPrimitive.Thumb
              ...
              onBlur={() => setDragging(-1)}
            />
          </TooltipTrigger>
          <TooltipContent side="top" className="z-[1000]">
            {_values[index].toFixed(2)}
          </TooltipContent>
        </Tooltip>
      ))}
    </SliderPrimitive.Root>
